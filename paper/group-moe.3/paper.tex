...
\begin{table}[h]
\centering
\caption{Three-way comparison on $S_3$ ternary task (num\_range=15). Mean $\pm$ std over 3 runs.}
\label{tab:threeway}
\begin{tabular}{lcc}
\toprule
Model & Complement (1$\to$5) & Composition (4$\to$2) \\
\midrule
\gmoe{} (irrep $\Rg$) & \textbf{91.8 $\pm$ 0.1\%} & \textbf{99.2 $\pm$ 0.5\%} \\
StandardMoE (learned $W$) & 88.5 $\pm$ 0.3\% & 98.4 $\pm$ 0.5\% \\
Baseline (no expert) & 86.8 $\pm$ 0.5\% & 96.6 $\pm$ 0.8\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Decomposition.} On the complement split, the group structure contributes +3.3pp beyond what the routing architecture alone provides (+1.7pp), for a total advantage of +5.0pp. On the easier composition split, the group contribution is +0.8pp vs.\ +1.8pp from routing. \textbf{The harder the generalization, the more the algebraic structure matters} (Figure~\ref{fig:threeway}).

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig3_threeway.png}
\caption{Three-way comparison on $S_3$ ternary task (num\_range=15). \textbf{Left:} complement split (1 ordering trains, 5 test). \textbf{Right:} composition split (transpositions train, 3-cycles test). Red annotations show the decomposition: +3.3pp from group structure, +1.7pp from routing architecture.}
\label{fig:threeway}
\end{figure}
