...
\begin{abstract}
Neural networks that encounter symmetry either enforce it rigidly in every layer or ignore it entirely. We propose \gmoe{}, a mixture-of-experts architecture where each expert implements a finite group representation in the irreducible representation (irrep) basis, and a learned router detects which symmetry applies. This gives models \emph{algebraic fixed-function units}---analogous to dedicated hardware accelerators---that the network learns to invoke selectively. We introduce the \emph{complement split}, a controlled evaluation methodology that isolates symmetry transfer from memorization, and validate the architecture on synthetic tasks. Our experiments demonstrate: (1)~complement transfer from seen to unseen orderings, with the routing architecture providing a robust $+$3.2pp advantage over plain baselines on $S_3$, (2)~near-perfect zero-shot compositional generalization from transpositions to 3-cycles ($98.7 \pm 0.8$\%), (3)~correct router discrimination between non-nested groups, and (4)~zero-degradation compatibility as a transformer FFN replacement. We find that the routing architecture consistently helps, while the fixed irrep matrices provide the strongest advantage on specific seeds but do not reliably outperform learned transforms across seeds. We analyze when algebraic structure outperforms lookup-table memorization and identify the combinatorial scaling regime where group experts should provide their greatest advantage.
\end{abstract}
